...
<html lang="en">
<head>
  <title>Subject View Record Page</title>
  <meta charset="utf-8">
...
if ($result->num_rows > 0) {
	...
	echo "<tr><th>Id</th><th>Name</th><th>Semister</th><th>Action</th></tr>";
    while($row = $result->fetch_assoc()) {
		$subject_id=$row['subject_id'];
        echo "<tr><td> " . $row["subject_id"]. "</td>";
		echo "<td> " . $row["Subject_name"]. "</td>"; 
		echo "<td> " . $row["semister"]. "</td>"; 
		echo "<td><a class='btn btn-primary' href='subject_update.php?id=$subject_id' >Update</a></td></tr>";
    }
	echo "</table>";
} else {
    echo "0 results";
